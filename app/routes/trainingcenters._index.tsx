...
export default function Tracks() {
  ...
  return (
    <>
      <Header />
      <h1 className="text-center text-4xl py-5">Training Centers</h1>
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  let tracks = null;
  if (search) {
    tracks = await getTrainingCenterSearch(search);
  } else {
    tracks = await getAllTraingCenters();
  }
  return { tracks };
}
